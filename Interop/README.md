...
Page statique (fetch asynchrone) croisant géoloc IP, disponibilité VéloStan'lib, météo et qualité de l'air, avec carte Leaflet centrée sur la position détectée.

## Fichiers
- `circulations.html` : page principale.
- `styles.css` : styles (media queries incluses).
...
- Carte Leaflet centrée sur la position détectée, popups stations (vélos/places) et liste triée par distance.
...
- Mise en forme responsive (media queries dans `styles.css`).
